# DSA Practice — SDE Interview Prep

Pattern-wise Data Structures & Algorithms practice repository, built as part of structured preparation for entry-level SDE roles. All solutions in Python 3.

**LeetCode:** [Daksh_Devyansh](https://leetcode.com/Daksh_Devyansh)

**Progress: 23 / 185 problems solved (12%)**

---

## 📌 Approach

Every problem follows a fixed discipline before code is written:
1. Identify the pattern
2. Justify why the pattern applies
3. State time/space complexity of the intended approach

Then implement, trace through an example, and check edge cases before submitting.

---

## ✅ Progress Tracker


### Two Pointers
| # | Problem | Status |
|---|---------|--------|
| 125 | Valid Palindrome | ✅ |
| 167 | Two Sum II - Input Array Is Sorted | ✅ |
| 11 | Container With Most Water | ⏳ Planned |

### Sliding Window
| # | Problem | Status |
|---|---------|--------|
| 643 | Maximum Average Subarray I | ⏳ Planned |
| 1343 | Number of Sub-arrays of Size K and Avg Greater Than or Equal to Threshold | ⏳ Planned |
| 3 | Longest Substring Without Repeating Characters | ⏳ Planned |
| 209 | Minimum Size Subarray Sum | ⏳ Planned |
| 424 | Longest Repeating Character Replacement | ⏳ Planned |
| 1004 | Max Consecutive Ones III | ⏳ Planned |
| 567 | Permutation in String | ⏳ Planned |

### Binary Search
| # | Problem | Status |
|---|---------|--------|
| 704 | Binary Search | ⏳ Planned |
| 35 | Search Insert Position | ⏳ Planned |
| 34 | Find First and Last Position of Element in Sorted Array | ⏳ Planned |
| 33 | Search in Rotated Sorted Array | ⏳ Planned |
| 153 | Find Minimum in Rotated Sorted Array | ⏳ Planned |
| 875 | Koko Eating Bananas | ⏳ Planned |
| 1011 | Capacity To Ship Packages Within D Days | ⏳ Planned |
| 540 | Single Element in a Sorted Array | ⏳ Planned |
| GFG-NthRoot | Find Nth Root of M | ⏳ Planned |
| GFG-KthTwoArrays | Kth Element of Two Arrays | ⏳ Planned |
| IB-AllocateBooks | Allocate Books | ⏳ Planned |
| SPOJ-AggressiveCows | Aggressive Cows | ⏳ Planned |
| GFG-MatrixMedian | Matrix Median | ⏳ Planned |
| 4 | Median of Two Sorted Arrays | ⏳ Planned |

### Fast & Slow Pointers
| # | Problem | Status |
|---|---------|--------|
| 141 | Linked List Cycle | ⏳ Planned |
| 202 | Happy Number | ⏳ Planned |
| 876 | Middle of the Linked List | ⏳ Planned |
| 142 | Linked List Cycle II | ⏳ Planned |

### Linked List Reversal & Manipulation
| # | Problem | Status |
|---|---------|--------|
| 206 | Reverse Linked List | ⏳ Planned |
| 234 | Palindrome Linked List | ⏳ Planned |
| 21 | Merge Two Sorted Lists | ⏳ Planned |
| 19 | Remove Nth Node From End of List | ⏳ Planned |
| 2 | Add Two Numbers | ⏳ Planned |
| 237 | Delete Node in a Linked List | ⏳ Planned |
| 160 | Intersection of Two Linked Lists | ⏳ Planned |
| 25 | Reverse Nodes in k-Group | ⏳ Planned |
| GFG-FlattenLL | Flattening of a LinkedList | ⏳ Planned |
| 61 | Rotate a LinkedList | ⏳ Planned |
| 138 | Copy List with Random Pointer | ⏳ Planned |

### Stack / Monotonic Stack / Queue
| # | Problem | Status |
|---|---------|--------|
| 20 | Valid Parentheses | ⏳ Planned |
| 496 | Next Greater Element I | ⏳ Planned |
| 739 | Daily Temperatures | ✅ |
| 84 | Largest Rectangle in Histogram | ✅ |
| 42 | Trapping Rain Water | ⏳ Planned |
| TUF-StackArr | Implement Stack using Arrays | ⏳ Planned |
| TUF-QueueArr | Implement Queue using Arrays | ⏳ Planned |
| 225 | Implement Stack using Queues | ⏳ Planned |
| 232 | Implement Queue using Stacks | ⏳ Planned |
| TUF-SortStack | Sort a Stack | ⏳ Planned |
| 155 | Min Stack | ⏳ Planned |
| 146 | LRU Cache | ⏳ Planned |
| 460 | LFU Cache | ⏳ Planned |
| 901 | Online Stock Span | ⏳ Planned |
| 994 | Rotting Oranges | ⏳ Planned |
| 239 | Sliding Window Maximum | ⏳ Planned |
| IB-NextSmaller | Next Smaller Element | ⏳ Planned |
| TUF-Celebrity | The Celebrity Problem | ⏳ Planned |

### Prefix Sum / Difference Array
| # | Problem | Status |
|---|---------|--------|
| 560 | Subarray Sum Equals K | ✅ |
| 238 | Product of Array Except Self | ✅ |
| 303 | Range Sum Query - Immutable | ✅ |

### Merge Intervals
| # | Problem | Status |
|---|---------|--------|
| 56 | Merge Intervals | ✅ |
| 57 | Insert Interval | ✅ |
| 435 | Non-overlapping Intervals | ✅ |

### Cyclic Sort
| # | Problem | Status |
|---|---------|--------|
| 268 | Missing Number | ✅ |
| 448 | Find All Numbers Disappeared in an Array | ✅ |
| 41 | First Missing Positive | ✅ |

### Hashing-Based Patterns
| # | Problem | Status |
|---|---------|--------|
| 49 | Group Anagrams | ✅ |
| 128 | Longest Consecutive Sequence | ✅ |
| 347 | Top K Frequent Elements | ✅ |

### Recursion Fundamentals
| # | Problem | Status |
|---|---------|--------|
| 344 | Reverse String | ✅ |
| 50 | Pow(x, n) | ✅ |
| 78 | Subsets | ✅ |
| 46 | Permutations | ✅ |

### Backtracking
| # | Problem | Status |
|---|---------|--------|
| 39 | Combination Sum | ✅ |
| 79 | Word Search | ⏳ Planned |
| 51 | N-Queens | ⏳ Planned |
| 40 | Combination Sum II | ⏳ Planned |
| 90 | Subsets II | ⏳ Planned |
| 131 | Palindrome Partitioning | ⏳ Planned |
| 60 | Permutation Sequence | ⏳ Planned |
| 37 | Sudoku Solver | ⏳ Planned |
| GFG-MColoring | M-Coloring Problem | ⏳ Planned |
| GFG-RatMaze | Rat in a Maze | ⏳ Planned |
| GFG-WordBreakPrint | Word Break (print all ways) | ⏳ Planned |

### Arrays (Extra)
| # | Problem | Status |
|---|---------|--------|
| 53 | Maximum Subarray | ⏳ Planned |
| 121 | Best Time to Buy and Sell Stock | ⏳ Planned |
| 31 | Next Permutation | ⏳ Planned |
| 73 | Set Matrix Zeroes | ⏳ Planned |
| 75 | Sort Colors | ⏳ Planned |
| 118 | Pascal's Triangle | ⏳ Planned |
| 48 | Rotate Matrix | ⏳ Planned |
| 287 | Find the Duplicate Number | ⏳ Planned |
| IB-RepeatMissing | Repeat and Missing Number Array | ⏳ Planned |
| 169 | Majority Element | ⏳ Planned |
| 229 | Majority Element II | ⏳ Planned |
| 62 | Grid Unique Paths | ⏳ Planned |
| 74 | Search a 2D Matrix | ⏳ Planned |
| 493 | Reverse Pairs | ⏳ Planned |
| 1 | Two Sum | ⏳ Planned |
| 18 | 4Sum | ⏳ Planned |
| IB-SubarrayXOR | Subarray with Given XOR | ⏳ Planned |

### Strings
| # | Problem | Status |
|---|---------|--------|
| 13 | Roman to Integer | ⏳ Planned |
| 14 | Longest Common Prefix | ⏳ Planned |
| 5 | Longest Palindromic Substring | ⏳ Planned |
| 8 | String to Integer (atoi) | ⏳ Planned |
| 151 | Reverse Words in a String | ⏳ Planned |
| 38 | Count and Say | ⏳ Planned |
| 165 | Compare Version Numbers | ⏳ Planned |
| 242 | Valid Anagram | ⏳ Planned |
| TUF-KMP | KMP Algorithm / LPS Array | ⏳ Planned |

### Dynamic Programming
| # | Problem | Status |
|---|---------|--------|
| 70 | Climbing Stairs | ⏳ Planned |
| 322 | Coin Change | ⏳ Planned |
| 1143 | Longest Common Subsequence | ⏳ Planned |
| 72 | Edit Distance | ⏳ Planned |
| 152 | Maximum Product Subarray | ⏳ Planned |
| 300 | Longest Increasing Subsequence | ⏳ Planned |
| TUF-MatrixChain | Matrix Chain Multiplication | ⏳ Planned |
| TUF-Knapsack01 | 0-1 Knapsack | ⏳ Planned |
| 64 | Minimum Path Sum | ⏳ Planned |
| 139 | Word Break | ⏳ Planned |
| 416 | Partition Equal Subset Sum | ⏳ Planned |
| 1547 | Minimum Cost to Cut a Stick | ⏳ Planned |
| TUF-EggDrop | Super Egg Drop | ⏳ Planned |
| 474 | Ones and Zeroes | ⏳ Planned |

### Heaps
| # | Problem | Status |
|---|---------|--------|
| 215 | Kth Largest Element in an Array | ⏳ Planned |
| 295 | Find Median from Data Stream | ⏳ Planned |
| CN-HeapImpl | Max/Min Heap Implementation | ⏳ Planned |
| CN-MergeKArrays | Merge K Sorted Arrays | ⏳ Planned |
| IB-MaxSumComb | Maximum Sum Combination | ⏳ Planned |

### Trees
| # | Problem | Status |
|---|---------|--------|
| 104 | Maximum Depth of Binary Tree | ⏳ Planned |
| 226 | Invert Binary Tree | ⏳ Planned |
| 98 | Validate Binary Search Tree | ⏳ Planned |
| 235 | Lowest Common Ancestor of a BST | ⏳ Planned |
| 102 | Binary Tree Level Order Traversal | ⏳ Planned |
| 543 | Diameter of Binary Tree | ⏳ Planned |
| 112 | Path Sum | ✅ |
| 199 | Binary Tree Right Side View | ✅ |
| 143 | Reorder List | ⏳ Planned |
| 94 | Inorder Traversal | ⏳ Planned |
| 144 | Preorder Traversal | ⏳ Planned |
| 145 | Postorder Traversal | ⏳ Planned |
| TUF-MorrisIn | Morris Inorder Traversal | ⏳ Planned |
| TUF-MorrisPre | Morris Preorder Traversal | ⏳ Planned |
| TUF-LeftView | Left View of Binary Tree | ⏳ Planned |
| TUF-TopView | Top View of Binary Tree | ⏳ Planned |
| TUF-BottomView | Bottom View of Binary Tree | ⏳ Planned |
| 987 | Vertical Order Traversal | ⏳ Planned |
| 103 | Binary Tree Zigzag Level Order Traversal | ⏳ Planned |
| 110 | Balanced Binary Tree | ⏳ Planned |
| 236 | Lowest Common Ancestor of a Binary Tree | ⏳ Planned |
| 545 | Boundary Traversal of Binary Tree | ⏳ Planned |
| 101 | Symmetric Tree | ⏳ Planned |
| 105 | Construct Binary Tree from Preorder and Inorder Traversal | ⏳ Planned |
| 106 | Construct Binary Tree from Inorder and Postorder Traversal | ⏳ Planned |
| 114 | Flatten Binary Tree to Linked List | ⏳ Planned |
| 124 | Binary Tree Maximum Path Sum | ⏳ Planned |
| 700 | Search in a Binary Search Tree | ⏳ Planned |
| 1008 | Construct Binary Search Tree from Preorder Traversal | ⏳ Planned |
| 230 | Kth Smallest Element in a BST | ⏳ Planned |
| TUF-KthLargeSmallBST | Kth Largest and Smallest Element in BST | ⏳ Planned |
| 173 | Binary Search Tree Iterator | ⏳ Planned |
| 297 | Serialize and Deserialize Binary Tree | ⏳ Planned |

### Graphs
| # | Problem | Status |
|---|---------|--------|
| TUF-BFSDFS | BFS & DFS Traversal Techniques | ⏳ Planned |
| 133 | Clone Graph | ⏳ Planned |
| 200 | Number of Islands | ⏳ Planned |
| 785 | Is Graph Bipartite | ⏳ Planned |
| TUF-CycleUndirected | Cycle Detection - Undirected Graph (BFS & DFS) | ⏳ Planned |
| TUF-CycleDirected | Cycle Detection - Directed Graph (BFS & DFS) | ⏳ Planned |
| TUF-TopoSort | Topological Sort / Kahn's Algorithm | ⏳ Planned |
| TUF-Prims | Prim's Algorithm (MST) | ⏳ Planned |
| TUF-Kruskals | Kruskal's Algorithm (MST) | ⏳ Planned |
| TUF-FloydWarshall | Floyd-Warshall Algorithm | ⏳ Planned |
| TUF-BellmanFord | Bellman-Ford Algorithm | ⏳ Planned |
| TUF-Dijkstra | Dijkstra's Algorithm | ⏳ Planned |

### Greedy
| # | Problem | Status |
|---|---------|--------|
| 455 | Assign Cookies | ⏳ Planned |
| GFG-NMeetings | N Meetings in One Room | ⏳ Planned |
| GFG-FracKnapsack | Fractional Knapsack | ⏳ Planned |
| GFG-JobSequencing | Job Sequencing Problem | ⏳ Planned |
| GFG-MinPlatforms | Minimum Platforms | ⏳ Planned |

### Trie
| # | Problem | Status |
|---|---------|--------|
| 208 | Implement Trie (Prefix Tree) | ⏳ Planned |
| TUF-TrieII | Implement Trie II | ⏳ Planned |
| 421 | Maximum XOR of Two Numbers in an Array | ⏳ Planned |
| 1707 | Maximum XOR With an Element From Array | ⏳ Planned |
| TUF-DistinctSubstr | Number of Distinct Substrings in a String | ⏳ Planned |
| TUF-LongestWordPrefix | Longest Word with All Prefixes | ⏳ Planned |


---

## 🗺️ Roadmap Ahead

- [ ] Advanced Topics (Bit Manipulation, Segment Trees, Union-Find, Design Problems)

---

## 📁 Structure

Each solved problem lives in its pattern folder. Number-prefixed filenames and the numbers are their leetcode problem numbers.
(e.g. `560_Subarray_Sum_Equals_K.py`) are matched most reliably — recommended
going forward:

```
dsa-practice/
├── Two Pointers/
│   └── 125_Valid_Palindrome.py
├── Sliding Window/
│   └── 3_Longest_Substring_Without_Repeating_Characters.py
├── Prefix Sum_or_Difference Array/
│   └── 560_Subarray_Sum_Equals_K.py
└── ...
```

---

*Last auto-generated: 2026-08-25*
